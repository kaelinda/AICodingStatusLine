# Codex CLI 状态栏 — 安装与配置

截至 2026-04-02，Codex 已提供原生 `tui.status_line`，还能启用实验性 hooks。本项目提供的是更强的增强层：在 Codex 原生能力之上，用 `tmux` 包装补充多行 bars、`5h` / `weekly`、Git diff、主题和宽度裁剪。

如果你只需要官方基础 footer，用 `codex-native`；如果你要更丰富的状态 HUD，用 `codex` tmux 增强模式。

---

## 数据来源

Codex session JSONL 文件（`~/.codex/sessions/`）中的 `token_count` 事件提供：

- 累计 token 用量（input / cached / output / reasoning / total）
- Context window 大小（如 258400）
- 5h 速率限制（`rate_limits.primary`）— 已用百分比 + Unix 时间戳重置时间
- weekly 速率限制（`rate_limits.secondary`）— 已用百分比 + Unix 时间戳重置时间，用于格式化绝对重置时间

模型名和推理努力等级从 `~/.codex/config.toml` 读取。

> **注意：** Codex 不提供 extra usage 数据，因此不显示 `extra` 段落。

---

## 模式概览

| 模式 | 安装命令 | 适用场景 |
|------|----------|----------|
| 原生 | `./install.sh --target codex-native` | 只想使用 Codex 官方 `tui.status_line` |
| 增强 | `./install.sh --target codex` | 需要 `5h` / `weekly`、多行 bars、主题、Git diff |
| 增强 + hooks | `./install.sh --target codex --with-hooks` | 需要增强状态栏，且想把实验性 hooks 的最近 Bash 状态接到状态栏 |

`notify bridge` 是可叠加能力，可以和原生或增强模式一起安装。

增强模式安装时还支持两种刷新频率设置方式：

- 显式参数：`./install.sh --target codex --refresh-interval 3`
- 交互式输入：`./install.sh --target codex --interactive`

### 原生模式写入内容

`codex-native` 会把以下配置写入 `~/.codex/config.toml`：

```toml
[tui]
status_line = ["model-with-reasoning", "context-remaining", "current-dir"]
```

### 实验性 hooks

Codex 官方还有实验性 hooks，可用于 SessionStart / Tool 前后 / Stop 等生命周期脚本，但它们不是 Claude 那种可直接自定义状态栏渲染的 `statusLine.command`。

启用示例：

```toml
[features]
codex_hooks = true
```

本项目可以通过 `--with-hooks` 一键安装 sidecar：

```bash
./install.sh --target codex --with-hooks
```

安装后会：

- 安装 `~/.codex/bin/codex-hook-sidecar`
- 生成或合并 `~/.codex/hooks.json`
- 写入 `~/.codex/config.toml` 的 `[features].codex_hooks = true`
- 在 tmux 增强模式下写入 `[statusline].show_hook_segment = true`

当前 sidecar 会把最近一次 hooks 事件写到缓存里，状态栏可显示：

- `hook bash run`
- `hook bash ok`
- `hook bash fail`
- `hook startup`
- `hook stop`

> 官方当前限制：`PreToolUse` / `PostToolUse` 的 `matcher` 虽然是正则，但运行时目前只会发出 `Bash`。

### 原生通知 / notify bridge

Codex 顶层还支持 `notify` 外部命令，用来在需要用户注意时把 JSON payload 交给本地脚本处理。本项目提供了 `codex-notify-bridge`，可与原生模式或 tmux 增强模式组合。

安装示例：

```bash
./install.sh --target codex-native --with-notify
./install.sh --target codex --with-hooks --with-notify
```

安装后会写入：

```toml
notify = ["~/.codex/bin/codex-notify-bridge"]

[tui]
notifications = true
```

bridge 会做三件事：

- 读取 Codex 发来的通知 payload，并将最近一次通知写入 `/tmp/codex/statusline-notify-cache.json`
- 在 macOS 上通过 `osascript` 转发桌面通知
- 在 Linux 上通过 `notify-send` 转发桌面通知
- 在 tmux 增强模式下，如果安装时使用 `--with-notify`，会自动开启 `show_notify_segment = true`，在概览行显示 `notify ...`

## 前置依赖

| 工具 | 用途 |
|------|------|
| `tmux` | 终端复用器，承载增强模式底部状态栏 |
| `jq` | 解析 session JSONL（仅增强模式需要） |
| Codex CLI | OpenAI 的 CLI 编码助手 |

---

## 安装

### 方式一：一键安装脚本（推荐）

```bash
git clone https://github.com/kaelinda/AICodingStatusLine.git
cd AICodingStatusLine
./install.sh --target codex
```

这会安装增强模式所需文件到 `~/.codex/bin/`：

| 文件 | 说明 |
|------|------|
| `codex-tmux` | tmux 启动器，创建会话并配置状态栏 |
| `codex-statusline` | 状态栏渲染脚本（从 session JSONL 读取数据） |
| `codex-tmux-status` | 兼容层（重定向到 codex-statusline） |

可追加选项：

```bash
# 同时安装 Claude Code 和 Codex
./install.sh --target both

# 只启用 Codex 原生 tui.status_line
./install.sh --target codex-native

# 启用 Codex tmux 增强模式，并安装实验性 hooks sidecar
./install.sh --target codex --with-hooks

# 给原生或增强模式叠加 notify bridge
./install.sh --target codex-native --with-notify
./install.sh --target codex --with-hooks --with-notify

# 指定主题、布局和进度条样式
./install.sh --target codex --theme dracula --layout bars --bar-style blocks

# 只卸载 Codex tmux 增强模式
./install.sh --uninstall --target codex

# 只移除 Codex 原生 tui.status_line 和 notify bridge
./install.sh --uninstall --target codex-native
```

> `./install.sh --uninstall` 不带 `--target` 时会按兼容模式全局清理 Claude Code 和 Codex；日常建议使用上面的目标化卸载命令。

## 更新

本项目没有单独的升级命令。更新方式就是拉最新代码后，按你当前使用的模式重新执行一次安装脚本。

```bash
git pull --ff-only

# 原生模式
./install.sh --target codex-native

# 原生模式 + notify
./install.sh --target codex-native --with-notify

# tmux 增强模式
./install.sh --target codex

# tmux 增强模式 + hooks
./install.sh --target codex --with-hooks

# tmux 增强模式 + hooks + notify
./install.sh --target codex --with-hooks --with-notify
```

### 方式二：手动安装

```bash
mkdir -p ~/.codex/bin

cp scripts/codex_tmux.sh ~/.codex/bin/codex-tmux
cp scripts/codex_statusline.sh ~/.codex/bin/codex-statusline
chmod +x ~/.codex/bin/codex-tmux ~/.codex/bin/codex-statusline
```

建议将 `~/.codex/bin` 加入 `PATH`：

```bash
echo 'export PATH="$HOME/.codex/bin:$PATH"' >> ~/.zshrc  # 或 ~/.bashrc
source ~/.zshrc
```

---

## 使用

### 原生模式

原生模式不需要额外启动器，直接启动 Codex 即可，底部由官方 `tui.status_line` 渲染。

### tmux 增强模式

```bash
# 完整路径
~/.codex/bin/codex-tmux

# 如果已在 PATH 中
codex-tmux

# 带环境变量启动
CODEX_MODEL_NAME=o3 CODEX_STATUSLINE_THEME=dracula codex-tmux
```

启动后会创建一个以项目目录命名的 tmux 会话（如 `codex-myproject`），底部显示状态栏。

---

## 显示段落

| 段落 | 含义 | 示例 |
|------|------|------|
| **Model** | 当前模型名称 | `gpt-5.4` |
| **CWD@Branch** | 当前目录名 + Git 分支，仓库有改动时追加 `(+N -N)`；可配置为仅显示分支 | `myapp@main (+3 -1)` / `branch:main` |
| **ctx** | 当前已用 / 总计 context window + 百分比 | `ctx 89k/258k 34%` |
| **eff** | 当前推理努力等级 | `low` / `medium` / `high` |
| **5h** | 5 小时速率限制剩余百分比 + 重置时间 | `5h 86% left 13:30` |
| **weekly** | 长周期额度（secondary rate limit）的剩余百分比 + 绝对重置时间 | `weekly 96% left 3/25 0:00 reset` |

剩余额度按阈值变色：🟢 >50% → 🟡 ≤50% → 🟠 ≤30% → 🔴 ≤10%

状态栏里的 `ctx` 表示 context window；`weekly` 表示长周期额度摘要。

---

## 配置

增强模式支持两种配置方式，**环境变量优先级高于 config.toml**。

如果你希望像图形化 footer 配置器那样边改边看效果，现在也可以直接用本地配置器：

```bash
./scripts/codex_statusline_cli.sh show
./scripts/codex_statusline_cli.sh preview
./scripts/codex_statusline_cli.sh configure
```

安装过 `./install.sh --target codex` 后，还会自动安装一个快捷入口：

```bash
~/.codex/bin/codex-statusline-config
```

`configure` 会在终端里即时刷新底部预览，预览本身直接走真实 `codex_statusline.sh` 渲染链路，因此看到的效果和实际 tmux 底栏一致。

### 环境变量

| 环境变量 | 默认值 | 说明 |
|----------|--------|------|
| `CODEX_STATUSLINE_SEGMENTS` | `model,eff,ctx,git,5h,7d` | 逗号分隔的段落列表：`model,eff,ctx,git,buddy,hook,notify,5h,7d`。既控制显示哪些段落，也控制紧凑布局和概览行里的顺序 |
| `CODEX_STATUSLINE_THEME` | `default` | 主题（与 Claude Code 相同的 9 种） |
| `CODEX_STATUSLINE_LAYOUT` | `bars` | 布局：`bars` 或 `compact` |
| `CODEX_STATUSLINE_BAR_STYLE` | `ascii` | 进度条样式（见 [README 进度条样式](../README.md#-布局与样式)），支持 `custom:填充:空白` 自定义 |
| `CODEX_STATUSLINE_GIT_DISPLAY` | `repo` | Git 段显示方式：`repo` 输出 `repo@branch`，`branch` 输出 `branch:<branch>` |
| `CODEX_STATUSLINE_REFRESH_INTERVAL` | 未设置 | 统一控制 tmux `status-interval` 与 session 用量缓存 TTL；设置后两者都使用该秒数 |
| `CODEX_STATUSLINE_SHOW_GIT_LINE` | `true` | 仅对 `bars` 布局生效，控制第 1 行 `repo@branch` 是否显示 |
| `CODEX_STATUSLINE_SHOW_OVERVIEW_LINE` | `true` | 仅对 `bars` 布局生效，控制第 2 行概览区是否显示。概览区左侧为 `model \| effort \| ctx`，右侧为可选 `buddy` |
| `CODEX_STATUSLINE_SHOW_BUDDY_SEGMENT` | `false` | 显示 buddy handoff 状态。`bars` 布局下位于概览行右侧，`compact` 下退化为普通 segment |
| `CODEX_STATUSLINE_BUDDY_CACHE_FILE` | `/tmp/codex/statusline-buddy-cache.json` | buddy 状态缓存文件 |
| `CODEX_STATUSLINE_BUDDY_TTL` | `600` | buddy 缓存可见时长，单位秒 |
| `CODEX_STATUSLINE_SHOW_HOOK_SEGMENT` | `false` | 显示最近一次 hooks sidecar 状态，如 `hook bash ok` |
| `CODEX_STATUSLINE_HOOK_CACHE_FILE` | `/tmp/codex/statusline-hook-cache.json` | hooks sidecar 写入的状态缓存文件 |
| `CODEX_STATUSLINE_HOOK_TTL` | `600` | hook 缓存可见时长，单位秒 |
| `CODEX_STATUSLINE_SHOW_NOTIFY_SEGMENT` | `false` | 显示最近一次 notify bridge 状态，如 `notify Waiting for approval` |
| `CODEX_STATUSLINE_NOTIFY_CACHE_FILE` | `/tmp/codex/statusline-notify-cache.json` | notify bridge 写入的状态缓存文件 |
| `CODEX_STATUSLINE_NOTIFY_TTL` | `600` | notify 缓存可见时长，单位秒 |
| `CODEX_STATUSLINE_MAX_WIDTH` | 终端宽度 | 强制指定宽度预算 |
| `CODEX_STATUSLINE_SESSION_DIR` | `~/.codex/sessions` | 会话目录覆盖 |
| `CODEX_STATUSLINE_TWO_WEEK_TIME_FORMAT` | `%-m/%-d %-H:%M reset` | `weekly` 时间格式，支持 `%y %Y %m %d %H %M %b %B` 与空格、`/`、`:`、`-` |
| `CODEX_MODEL_NAME` | 从 config.toml | 覆盖模型名 |
| `CODEX_EFFORT_LEVEL` | 从 config.toml | 覆盖推理努力等级 |

### config.toml 配置（推荐持久化方式）

在 `~/.codex/config.toml` 中添加项目私有的 `[statusline]` 段落：

```toml
[statusline]
theme = "dracula"
layout = "bars"
bar_style = "blocks"
segments = "ctx,model,git,5h,7d,eff"
git_display = "branch"
refresh_interval = 5
show_git_line = true
show_overview_line = true
show_buddy_segment = true
show_hook_segment = true
show_notify_segment = true
two_week_time_format = "%y-%m-%d %H:%M"
```

支持的键：

| 键 | 可选值 | 说明 |
|----|--------|------|
| `theme` | `default`、`forest`、`dracula`、`monokai`、`solarized`、`ocean`、`sunset`、`amber`、`rose` | 配色主题 |
| `layout` | `compact`、`bars` | 布局模式 |
| `bar_style` | `ascii`、`dots`、`squares`、`blocks`、`braille`、`shades`、`diamonds`、`custom:X:Y` | 进度条样式 |
| `git_display` | `repo`、`branch` | Git 段显示方式 |
| `refresh_interval` | 正整数秒 | 统一控制 tmux `status-interval` 与 session 用量缓存 TTL |
| `show_git_line` | `true`、`false` | 仅对 `bars` 布局生效，控制 git 行是否显示 |
| `show_overview_line` | `true`、`false` | 仅对 `bars` 布局生效，控制概览行是否显示 |
| `show_buddy_segment` | `true`、`false` | 显示 buddy handoff 状态。`bars` 布局下放在概览行右侧 |
| `show_hook_segment` | `true`、`false` | 在概览行或 compact 中显示最近一次 hooks 状态 |
| `show_notify_segment` | `true`、`false` | 在概览行或 compact 中显示最近一次 notify 状态 |
| `two_week_time_format` | 合法 `strftime` 子集 | `weekly` 绝对时间格式 |

**完整 config.toml 示例：**

```toml
model = "gpt-5.4"
model_reasoning_effort = "high"

[statusline]
theme = "ocean"
layout = "bars"
bar_style = "diamonds"
refresh_interval = 5
show_git_line = true
show_overview_line = true
show_buddy_segment = true
show_hook_segment = true
two_week_time_format = "%m/%d %H:%M"
```

---

## 宽度自适应

状态栏会根据终端宽度自动裁剪，按以下优先级逐步缩减：

| 优先级 | 操作 |
|--------|------|
| 1 | 隐藏 weekly 完整时间，降级为短日期 |
| 2 | 隐藏 5h 重置时间 |
| 3 | 隐藏 Git diff 统计 |
| 4 | 移除整个 weekly 段落 |
| 5 | 用 `...` 截断 Git 段落 |

在 `bars` 布局中，概览行会优先保留右侧 buddy 状态词，再裁掉 buddy 摘要，之后才开始压缩左侧概览信息；5h / weekly 进度条行会先缩短时间文本，最后才缩小进度条宽度。

用量行的视觉层级做了专门优化：`5h` 与 `weekly` 标签使用不同强调色，百分比加粗显示，进度条空槽使用独立轨道色，避免和普通弱化文字混在一起。颜色仍按额度语义变化，剩余额度越低越接近告警色。

---

## 缓存

| 缓存路径 | TTL | 说明 |
|----------|-----|------|
| `/tmp/codex/statusline-session-cache.json` | 10 秒，或 `refresh_interval` 指定值 | 从 session JSONL 解析的 token/rate_limits 数据 |
| `/tmp/codex/statusline-buddy-cache.json` | 600 秒 | buddy handoff 状态缓存 |

可通过 `CODEX_STATUSLINE_CACHE_FILE` 环境变量覆盖缓存路径。若你觉得用量刷新不够及时，优先设置 `CODEX_STATUSLINE_REFRESH_INTERVAL` 或 `[statusline].refresh_interval`，它会同时影响 tmux 重绘频率和 session cache TTL。

### Buddy 缓存格式

buddy 读取的是一个普通 JSON 文件。任何外部脚本都可以写这个文件，状态栏只负责读取和渲染。

示例：

```json
{
  "status": "needs_input",
  "summary": "Waiting for confirmation",
  "updated_at": 1773350100,
  "source": "sidecar"
}
```

字段说明：

- `status`：推荐值为 `idle`、`ok`、`done`、`blocked`、`needs_input`
- `summary`：可选摘要。`bars` 概览行宽度足够时会显示，宽度不足时优先裁掉它
- `updated_at`：Unix epoch 秒级时间戳，用于 TTL 判断
- `source`：可选，仅用于记录事件来源，当前不直接显示

---

## tmux 多行布局（bars）

`bars` 布局在 tmux 中显示为 4 行：

```
第 1 行：myapp@main
第 2 行：gpt-5.4 | eff high | ctx 89k/258k 34%                      buddy needs input Waiting for confirmation
第 3 行：5h 86% left [████████░░░░░░░░░░░░] 13:30 reset
第 4 行：weekly 96% left [█░░░░░░░░░░░░░░░░░░░] 3/25 0:00 reset
```

`codex_tmux.sh` 会自动检测 `bars` 布局并配置 tmux 四行状态栏（`status 4`），无需手动设置。buddy 不会增加新行，只占用第 2 行右侧区域。

可通过两个开关动态隐藏前两行，并自动缩减 tmux 状态栏高度：

```toml
[statusline]
layout = "bars"
show_git_line = false
show_overview_line = true
```

常见组合：

```text
show_git_line=true,  show_overview_line=true   -> 4 行: git / overview / 5h / weekly
show_git_line=false, show_overview_line=true   -> 3 行: overview / 5h / weekly
show_git_line=true,  show_overview_line=false  -> 3 行: git / 5h / weekly
show_git_line=false, show_overview_line=false  -> 2 行: 5h / weekly
```

`codex_statusline.sh --line N` 的内部编号保持不变：

```text
1 = git
2 = overview
3 = 5h
4 = weekly
```

如果第 1 或第 2 行被隐藏，对应 `--line` 会返回空字符串；tmux 启动器会自动把可见行映射到新的显示顺序。

布局来源优先级：环境变量 `CODEX_STATUSLINE_LAYOUT` > `config.toml` 的 `[statusline].layout` > 默认 `bars`。

---

## 冒烟测试

```bash
# 基本测试
CODEX_MODEL_NAME=gpt-5.4 ./codex_statusline.sh .

# 指定宽度
CODEX_STATUSLINE_MAX_WIDTH=80 ./codex_statusline.sh .

# 指定主题
CODEX_STATUSLINE_THEME=dracula ./codex_statusline.sh .

# bars 布局
CODEX_STATUSLINE_LAYOUT=bars ./codex_statusline.sh .
```

---

## 常见问题

<details>
<summary><strong>只显示 <code>5h -</code> / <code>weekly -</code>，或 bars 布局显示 <code>unavailable</code>，是什么意思？</strong></summary>

这表示当前没有拿到可用的额度数据，不是“额度为 0”。常见原因有两种：

- 找不到有效的 session JSONL 文件，或文件中没有 `token_count` 事件。
- session 中虽然有 `token_count`，但 `payload.rate_limits` 为 `null`。这在较新的 Codex CLI 中可能出现。

先确认 Codex CLI 已运行过至少一次对话，再检查 `~/.codex/sessions/` 目录下最新 `.jsonl` 是否包含 `token_count`。如果 `rate_limits` 缺失，状态栏会保留 `ctx` 等本地可得信息，并把 `5h` / `weekly` 明确标记为 `unavailable`。
</details>

<details>
<summary><strong>tmux 状态栏显示乱码？</strong></summary>

状态栏会自动检测 tmux 环境并使用 `#[fg=...]` 格式。如果仍有问题，检查终端是否支持 256 色或 true color：`echo $TERM` 应为 `xterm-256color` 或 `tmux-256color`。
</details>

<details>
<summary><strong>bars 布局只显示一行？</strong></summary>

确认你使用的是 `codex-tmux` 启动器，它会自动配置 tmux 多行状态栏。如果直接在已有 tmux 会话中使用，需要手动设置 `tmux set status 4`。
</details>

<details>
<summary><strong>如何切换进度条样式？</strong></summary>

在 `~/.codex/config.toml` 中修改：

```toml
[statusline]
bar_style = "diamonds"
```

或使用自定义字符：

```toml
[statusline]
bar_style = "custom:▰:▱"
```

修改后重新进入 tmux 会话即可生效。
</details>

<details>
<summary><strong>如何完全不显示 bars 进度条？</strong></summary>

将布局设为 `compact`（默认值），所有信息会压缩在一行内显示。
</details>
